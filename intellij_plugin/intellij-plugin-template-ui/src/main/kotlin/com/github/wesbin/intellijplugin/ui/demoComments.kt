...
fun demoComments(): DialogPanel {
    return panel {
        row("Row1:") {
            textField().comment("Comment to textField1").text("textField1")
            textField().text("textField2")
            textField().comment("Comment to textField3").text("textField3")
        }
        ...
        row("Row3:") {
            comment(
                "Comments can be an html text with some clickable <a href='link'>link</a> and even <a href='several links'>several links</a>"
            ) {
                Messages.showMessageDialog(
                    "Link '${it.description}' is clicked", "Message", null
                )
            }
        }
